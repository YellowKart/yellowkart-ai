  ...
  return (
    <Page>
      <Container maxWidth="md">
        <Typography variant="h3" sx={{ fontWeight: 800, color: '#232F3E', mb: 1 }}>
          Requirements
        </Typography>
        <Typography sx={{ color: '#5c6670', mb: 3, maxWidth: 640 }}>
        ...
        <Box sx={{ mb: 4, p: 2, borderTop: '2px solid #FF9900' }}>
          <Typography variant="h6" sx={{ mb: 1, fontWeight: 700 }}>
            Upload handwritten list
          </Typography>
          <Typography sx={{ mb: 2, color: '#5c6670', fontSize: 14 }}>
          ...
          <TextField
            fullWidth
            size="small"
            label="Optional hint (any Indian language)"
            value={aiHint}
          ...
          {aiResult && (
            <Box sx={{ mt: 2 }}>
              <Typography sx={{ mb: 1 }}>{aiResult.replyMessage}</Typography>
              {aiResult.lines.map((line: ListLineSuggestion, index: number) => (
                <Box key={`${line.rawText}-${index}`} sx={{ mb: 2 }}>
                  ...
                  {line.suggestions?.length ? (
                    ...
                    >
                      {line.suggestions.map((suggestion, sIndex) => (
                        <MenuItem key={suggestion.id} value={sIndex}>
                          {suggestion.name}
                    ...
                    <Typography color="text.secondary">No catalog match</Typography>
                  )}
                </Box>
              ))}
        ...
        {groups.map((group) => (
          <CategoryBlock key={group.category}>
            <Typography variant="h5" sx={{ fontWeight: 700, color: '#232F3E', mb: 1 }}>
              {group.category}
            </Typography>
            <Divider sx={{ mb: 1 }} />
        ...
        {selectedCount > 0 && (
          <Box sx={{ mt: 2, color: '#5c6670', fontSize: 13 }}>
            Selected:{' '}
