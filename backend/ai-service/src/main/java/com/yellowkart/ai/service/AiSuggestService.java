...
public class AiSuggestService {
    ...
    public SuggestResponse suggestFromText(String query, Integer limit) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "suggestFromText")) {
            AiInterpretation interpretation = openAiClient.interpretText(query);
            return toResponse("text", interpretation, limit);
        }
    }

    public SuggestResponse suggestFromImage(byte[] imageBytes, String contentType, String fileName, String hint, Integer limit) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "suggestFromImage")) {
    ...
    public SuggestResponse suggestFromVoice(byte[] audioBytes, String contentType, String fileName, String transcript, Integer limit) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "suggestFromVoice")) {
    ...
    public ListSuggestResponse suggestFromHandwrittenList(byte[] imageBytes, String contentType, String fileName, String hint, Integer limitPerLine) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "suggestFromHandwrittenList")) {
            HandwrittenListExtraction extraction = openAiClient.analyzeHandwrittenList(imageBytes, contentType, fileName, hint);
            ...
            for (HandwrittenListItem item : extraction.items) {
                try (YkTrace.Scope __ykBlock1 = YkTrace.block("AiSuggestService", "suggestFromHandwrittenList", "b1")) {
                    AiInterpretation interpretation = openAiClient.interpretText(buildLineQuery(item));
                    List<ProductSuggestion> suggestions = productMatcher.match(interpretation, perLine);
                    localizeReasons(suggestions, item.detectedLanguage != null ? item.detectedLanguage : extraction.detectedLanguage);
    ...
    public WebsiteCatalogResponse extractWebsiteCatalog(WebsiteCatalogRequest request) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "extractWebsiteCatalog")) {
            if (request == null) {
                try (YkTrace.Scope __ykBlock1 = YkTrace.block("AiSuggestService", "extractWebsiteCatalog", "b1")) {
                    throw new IllegalArgumentException("request is required");
                }
            }
            if (request.brand == null || request.brand.isBlank()) {
                try (YkTrace.Scope __ykBlock2 = YkTrace.block("AiSuggestService", "extractWebsiteCatalog", "b2")) {
                    throw new IllegalArgumentException("brand is required");
                }
            }
            if (request.urls == null || request.urls.isEmpty()) {
                try (YkTrace.Scope __ykBlock3 = YkTrace.block("AiSuggestService", "extractWebsiteCatalog", "b3")) {
    ...
    public String providerStatus() {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "providerStatus")) {
            return openAiClient.providerStatus();
        }
    }

    public boolean isOpenAiConfigured() {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "isOpenAiConfigured")) {
            return openAiClient.isConfigured();
        }
    }

    private String buildLineQuery(HandwrittenListItem item) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "buildLineQuery")) {
            StringBuilder sb = new StringBuilder();
            if (item.queryText != null && !item.queryText.isBlank()) {
                try (YkTrace.Scope __ykBlock2 = YkTrace.block("AiSuggestService", "buildLineQuery", "b2")) {
                    sb.append(item.queryText);
                }
            } else if (item.rawText != null) {
                try (YkTrace.Scope __ykBlock1 = YkTrace.block("AiSuggestService", "buildLineQuery", "b1")) {
                    sb.append(item.rawText);
                }
            }
            if (item.brandHint != null && !item.brandHint.isBlank()) {
                try (YkTrace.Scope __ykBlock3 = YkTrace.block("AiSuggestService", "buildLineQuery", "b3")) {
    ...
    private SuggestResponse toResponse(String inputType, AiInterpretation interpretation, Integer limit) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "toResponse")) {
            ...
            response.replyMessage = interpretation.replyMessage != null && !interpretation.replyMessage.isBlank() ? interpretation.replyMessage : interpretation.summary;
    ...
    private void localizeReasons(List<ProductSuggestion> suggestions, String lang) {
        try (YkTrace.Scope __ykMethod = YkTrace.method("AiSuggestService", "localizeReasons")) {
            if (suggestions == null || lang == null)
                return;
            String code = lang.toLowerCase(Locale.ROOT);
            if (code.startsWith("en"))
                return;
            for (ProductSuggestion suggestion : suggestions) {
                try (YkTrace.Scope __ykBlock1 = YkTrace.block("AiSuggestService", "localizeReasons", "b1")) {
                    String matched = suggestion.matchedKeywords == null || suggestion.matchedKeywords.isEmpty() ? suggestion.category : String.join(", ", suggestion.matchedKeywords.stream().limit(4).toList());
