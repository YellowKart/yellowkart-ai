...
const RequirementsScreen = ({ navigation }: any) => {
  ...
  const pickImage = async (fromCamera: boolean) => {
    try {
      ...
      const result = fromCamera
        ? await ImagePicker.launchCamera({ mediaType: 'photo', quality: 0.7 })
        : await ImagePicker.launchImageLibrary({ mediaType: 'photo', quality: 0.7 });
      if (result.didCancel || !result.assets?.[0]?.uri) return;
      await analyzeUri(result.assets[0].uri);
    } catch (error) {
      Alert.alert(
        'Image picker unavailable',
        'Paste a local image URI below, or install react-native-image-picker for camera/gallery.'
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      <SectionList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Optional AI hint (any Indian language)"
              value={aiHint}
            ...
            <TextInput
              style={styles.input}
              placeholder="Or paste image file URI"
              value={manualUri}
              onChangeText={setManualUri}
            />
            <TouchableOpacity
              style={styles.secondaryBtn}
              disabled={aiLoading || !manualUri}
              onPress={() => analyzeUri(manualUri)}
            >
        ...
        renderItem={({ item }) => (
          <View style={styles.itemRow}>
            ...
            <TextInput
              ...
              onChangeText={(value) => setQty(item.id, value)}
            />
          </View>
        )}
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[styles.primaryBtn, (submitting || selectedCount === 0) && styles.disabled]}
      ...
      <Modal visible={reviewOpen} animationType="slide">
        <ScrollView style={styles.modal}>
          ...
          {aiResult?.lines.map((line, index) => (
            <View key={`${line.rawText}-${index}`} style={styles.reviewLine}>
              <Text style={styles.itemName}>
                {line.rawText}
                {line.quantity ? ` · qty ${line.quantity}` : ''}
              </Text>
              {line.suggestions?.length ? (
                line.suggestions.map((suggestion, sIndex) => (
                  <TouchableOpacity
                    key={suggestion.id}
                    style={[
                      styles.choice,
                      (lineSelections[index] ?? 0) === sIndex && styles.choiceActive,
                    ]}
                    onPress={() =>
                ...
                <Text style={styles.itemBrand}>No catalog match</Text>
              )}
            </View>
          ))}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: '800', color: '#232F3E', marginBottom: 6 },
  subtitle: { color: '#5c6670', marginBottom: 12 },
  input: {
    backgroundColor: '#fff',
    borderRadius: 8,
    borderWidth: 1,
  ...
  itemName: { fontWeight: '600', color: '#232F3E', marginBottom: 2 },
  itemBrand: { color: '#6b7280', fontSize: 13 },
  qtyInput: {
    width: 72,
    backgroundColor: '#fff',
    borderWidth: 1,
  ...
  secondaryBtnText: { color: '#232F3E', fontWeight: '600' },
  disabled: { opacity: 0.5 },
  preview: { width: '100%', height: 160, marginTop: 8, borderRadius: 8 },
  modal: { flex: 1, padding: 16, backgroundColor: '#F7F4EF' },
  reviewLine: { marginBottom: 16 },
  choice: {
    backgroundColor: '#fff',
    padding: 10,
    borderRadius: 8,
  ...
  choiceActive: { borderColor: '#FF9900', backgroundColor: '#FFF7E8' },
});
